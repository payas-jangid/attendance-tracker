  ...
  return (
    ...
    >
      <View className="px-6 py-4">
        <Text className="text-3xl font-bold text-amber-100">Test Scores</Text>
        <Text className="text-amber-200/70 mt-1">
      ...
      <FlatList
        ...
        renderItem={({ item }: { item: SubjectItem }) => (
          <View className="bg-amber-50 mx-5 my-3 p-5 rounded-2xl shadow-sm">
            ...
            <Text className="text-xl font-bold text-amber-950 mb-4 border-b border-amber-200/50 pb-2">
              {item.name}
            </Text>
            ...
            <View className="flex-row flex-wrap justify-between gap-y-4">
              ...
              <View className="w-[47%]">
                ...
                <TextInput
                  value={String((parseInt(item.scores.end_sem) || 0) + (parseInt(item.scores.mid_sem) || 0) + (parseInt(item.scores.quiz1) || 0) + (parseInt(item.scores.quiz2) || 0))}
                  onChangeText={(text) => updateScore(item.id, "end_sem", text)}
                  placeholder="e.g. 75"
                  keyboardType="numeric"
                  className="bg-white p-3 rounded-xl border border-amber-200 text-amber-950 font-medium"
